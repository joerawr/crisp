...
Usage: `/crisp 1-5`, or `/crisp off` (also "stop crisp" / "normal mode"); set a default via `CRISP_DEFAULT_LEVEL` or `~/.config/crisp/config.json`, and the statusline shows `[CRISP:n]`. Inspired by ponytail (hook architecture), caveman (leveled brevity, minus the broken-English persona), and humanizer (the floor).
